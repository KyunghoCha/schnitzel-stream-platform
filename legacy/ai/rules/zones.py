# Docs: docs/implementation/20-zones-rules/README.md
...
def _now_ts() -> float:
    # 캐시 TTL 계산용 현재 시각(UTC timestamp)
    return datetime.now(timezone.utc).timestamp()


def point_in_polygon(point: tuple[float, float], polygon: list[list[float]]) -> bool:
    # ray casting 알고리즘(odd-even rule)
    # - polygon: [[x, y], ...]
    # - 경계선 위 점은 inside로 처리
    if len(polygon) < 3:
        return False
    ...
    for i in range(n):
        ...
        # 수평 경계선 위에 정확히 놓인 경우
        if math.isclose(y1, y2, abs_tol=1e-9) and math.isclose(y, y1, abs_tol=1e-9) and min(x1, x2) <= x <= max(x1, x2):
            return True
...
def evaluate_zones(
    ...
    # zone inside 판정
    # - rule_point 계산 후 각 zone polygon 내부 여부 체크
...
class ZoneCache:
    ...
    def get_stale(self) -> list[dict[str, Any]]:
        """TTL 무시, 캐시에 저장된 zones 반환 (없으면 빈 리스트)."""
        with self._lock:
            if self._zones is None:
                return []
            return list(self._zones)
...
class ZoneEvaluator:
    ...
    def apply(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if zones:
            payload["zone"] = evaluate_zones(event_type, bbox, zones, self.rule_map)
        else:
    ...
    def _load_for_apply(self) -> list[dict[str, Any]]:
        ...
        # 의도: API 기반 zone 조회는 추론 경로를 블로킹하지 않는다.
        # apply()에서는 캐시(stale 포함)를 즉시 사용하고, 새 fetch는 백그라운드에서 수행한다.
