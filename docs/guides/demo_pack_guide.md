...
This guide defines the reproducible demo flow for the P11 demo package.

## Scope

- S1: in-proc baseline (`demo_inproc.yaml`)
- S2: durable enqueue + drain/ack (`demo_durable_*`)
- S3: webcam pipeline (`demo_webcam.yaml`)
...
Webcam profile (includes webcam):
...
- `20`: webcam run failure in webcam profile
...
```bash
# Linux/macOS
rm -f outputs/queues/demo_pack.sqlite3
```

```powershell
# Windows PowerShell
Remove-Item outputs/queues/demo_pack.sqlite3 -ErrorAction SilentlyContinue
```
...
Expected output:
- drain lines prefixed with `DEMO_DRAIN`
...
3. Durable replay seems inconsistent:
   - Remove `outputs/queues/demo_pack.sqlite3` and rerun S2.
4. Command not found for `pytest`/`pip`:
   - Verify active Python environment and PATH.

## 한국어

이 가이드는 P11 데모 패키지 기준의 데모 재현 절차를 정의합니다.

## 범위

- S1: in-proc 기본선 (`demo_inproc.yaml`)
- S2: durable enqueue + drain/ack (`demo_durable_*`)
- S3: 웹캠 파이프라인 (`demo_webcam.yaml`)
