...
def get_mediamtx_key() -> str:
    ...
    if system == "windows":
        # Intent: windows arm64 has stable x64 emulation path.
...
def count_packets(log_dir: Path) -> int:
...
def wait_for_count(log_dir: Path, target_count: int, timeout_sec: float = 15.0) -> bool:
    ...
    while time.monotonic() - start < timeout_sec:
        if count_packets(log_dir) >= target_count:
            return True
        time.sleep(0.5)
    return False
...
def _resolve_video() -> Path:
    ...
    if not mp4s:
        raise FileNotFoundError(f"No test video found at {video_path} or in {sample_dir}")
    return mp4s[0]
...
def run_e2e_test(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"Error: Failed to setup MediaMTX: {exc}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        # 4) Wait for initial packets
        print("Waiting for initial packets...")
        wait_for_count(log_dir, 3, timeout_sec=30)
        count_before = count_packets(log_dir)
        print(f"count_before={count_before}")

        # 5) Kill ffmpeg to simulate disconnect
        ...
        recovered = wait_for_count(log_dir, count_before + 3, timeout_sec=30)
        count_after = count_packets(log_dir)
        ...
        for name in ("pipeline", "ffmpeg", "mediamtx"):
            if name in pids:
                stop_process(pids[name])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-dir",
        default=str(Path(tempfile.gettempdir()) / "rtsp_stability_test"),
        help="directory for logs and temporary files",
    )
